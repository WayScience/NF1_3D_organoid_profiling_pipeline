...
# Force BaSiC to use CPU-based SCIPY backend (avoids JAX dependency issues)
os.environ["BASIC_DCT_BACKEND"] = "SCIPY"
...
def read_image_stack(image_input: pathlib.Path | np.ndarray) -> np.ndarray:
    """Load a TIFF file or return a float32 array if already provided.

    Args:
        image_input (pathlib.Path | np.ndarray): pathlib.Path to a TIFF file or an already-loaded NumPy array.
...
def apply_basicpy_correction(
    image_input: pathlib.Path | np.ndarray,
    ...
    Args:
        image_input (pathlib.Path | np.ndarray): pathlib.Path to the TIFF file or a preloaded NumPy array.
        out_dir (pathlib.Path | None, optional): Directory to save the corrected TIFF file. Defaults to None.
...
def process_channel_stack(
    folder: pathlib.Path, out_well_dir: pathlib.Path, channel: str
) -> None:
    """Process all TIFF files for a given channel in a well/FOV folder.

    Args:
        folder (pathlib.Path): Path to the well/FOV folder containing TIFF files.
        out_well_dir (pathlib.Path): Directory to save the corrected TIFFs for this channel.
    ...
    for tif in tqdm.tqdm(tifs, desc=f"{folder.name} | {channel}", leave=False):
...
def process_well_fov(
    ...
    Args:
        folder (pathlib.Path): Path to the well/FOV folder containing TIFF files.
        out_dir (pathlib.Path): Directory where corrected TIFFs will be saved.
...
if in_notebook:
    ...
    patient = (
        "NF0037_T1"  # Change this to the desired patient ID when running in a notebook
    )
else:
...
image_base_dir = bandicoot_check(
    pathlib.Path(os.path.expanduser(f"~/mnt/bandicoot/NF1_organoid_data/data/")),
    root_dir=root_dir,
).resolve(strict=True)

# Resolve patient directory
patient_dir = pathlib.Path(f"{image_base_dir}/data/{patient}/zstack_images").resolve()
...
with tqdm.tqdm(total=len(well_fov_folders), desc="Processing Wells/FOVs") as pbar:
